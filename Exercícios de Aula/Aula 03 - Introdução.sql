...
SELECT count(*) FROM tb_produtos ; -- qnt, contagem

SELECT count(categoria_id) FROM tb_produtos;

SELECT sum(preco) FROM tb_produtos; -- soma

SELECT avg(preco) FROM tb_produtos; -- média total

SELECT categoria_id, avg(preco) as Média FROM tb_produtos
GROUP BY categoria_id; -- trás a média de cada categoria
...
ON tb_produtos.categoria_id = tb_categorias.id; -- relação
